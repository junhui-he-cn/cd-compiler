...
- `include/FrontendSession.hpp`, `src/FrontendSession.cpp`: import-aware source
  ...
  every `Program` snapshot carries a value copy of the graph for later
  semantic consumers; every file-backed, stdin, and virtual source is a
  module, and CLI files are entry modules in command-line order. The normal
  CLI requires source files; `--lsp` keeps its separate stdio protocol, and
  the in-memory `FrontendSession::loadStdin()` API remains available for LSP
  and internal tests.
- `include/ModuleInterface.hpp`, `src/ModuleInterfaceEmitter.cpp`: in-memory
  ...
  public-shape consumption reads those records after graph dependency-ordered
  preflight; importer operations no longer locate or check dependency bodies;
  the dependency body remains the current interface producer;
  `moduleInterfaceMismatchCount()` validates graph identity, dependency order,
  canonical vectors, and public-name coverage after the snapshot is complete.
- `include/ModuleInterfaceArtifact.hpp`, `src/ModuleInterfaceArtifact.cpp`:
  strict `cdi 0.1` sidecars containing complete public type shape, generic
  constraints, struct fields/methods, enum variants, dependency interface
  hashes, and cross-module linkage names; snapshot-local IDs are restored only
  after a validated sidecar is loaded.
- `include/ModuleCache.hpp`, `src/ModuleCache.cpp`: stable local module-product
  cache keys, strict `cdbc-cache 0.2` manifests, relative sidecar paths,
  dependency/public-interface invalidation, and per-module rebuild
  measurement.
- `include/DeclarationIndex.hpp`, `src/DeclarationIndex.cpp`: snapshot-local
  ...
  index records;
  literal, variant, record, OR, guard, coverage, and binding pattern lowering
  consume literal-pattern, variant-pattern, record-pattern, OR-pattern, guard,
  coverage, and binding records;
...
- `include/LanguageServer.hpp`, `src/LanguageServer.cpp`: stdio JSON-RPC/LSP
  ...
  open-document workspace analysis, and source-local/cross-module definition
  navigation.
...
- A resolved direct call to a local function with captured bindings invalidates active nullable narrowing for those visible captured bindings after argument checking. Non-capturing calls keep unrelated facts; struct-method effects remain conservative and are deferred to a later M2A slice.
- An indirect or dynamic function call, including a function alias or function-valued binding that is not resolved to a direct `FunctionStmt`, invalidates all active nullable narrowing after successful argument checking. Callback-capable native calls use the same all-fact invalidation rule; non-callback native calls remain unchanged.
- A successful callback-capable native call (`map`, `filter`, `flatMap`, `any`, `all`, `count`, `find`, `findIndex`, or `reduce`, including member-call sugar) invalidates all active nullable narrowing after callback checking. Native calls without callbacks, namespace calls, enum variants, and struct methods are outside this rule.
- A successful resolved struct-method call invalidates all active nullable narrowing because methods may mutate receiver fields or top-level state; this is a conservative call-effect rule, not field/index narrowing. Unresolved member calls and method dispatch extensions remain outside the current model.
- A supported truthiness condition such as `if (value)` narrows a supported nullable target only in the then branch because `nil` is falsey. The else branch remains conservative because the non-nil value `false` is also falsey; `0` and empty strings are truthy. This rule composes with supported `&&` then-branch guards, while unary `!` and truthiness-based else narrowing remain unsupported.
- A no-`else` `if` whose `then` statement is guaranteed not to fall through (a return, returning block, or returning exhaustive match) carries the condition's `else` narrowing into the continuation. Facts are restored from before the terminating branch so mutations on that dead path do not invalidate the continuation; explicit-`else` joins where both arms fall through remain conservative when no compatible fact is proven on both sides, while loops and unresolved dynamic index expressions remain conservative.
- An explicit-`else` `if` whose one arm is guaranteed not to fall through carries the other arm's final nullable flow facts into the continuation. When both arms fall through, only facts present with compatible types in both final branch states survive; branch-local proofs or invalidated facts are discarded. Both arms are checked in isolated branch states, and loops and unresolved dynamic index expressions remain conservative.
- A `while` condition's true-branch nullable narrowing is active while checking its body. When the condition contributes an else fact and the body contains no `break` targeting the current loop, that else fact survives a normal condition-failure exit; otherwise loop exits remain conservative. `continue`, returns, and breaks belonging to nested loops do not block this rule; C-style `for` and `for-in` loop-exit joins remain conservative.
- A C-style `for` condition's true-branch nullable narrowing is active while checking its body. When the body contains no `break` targeting the current loop, the condition's else fact survives a normal condition-failure exit; initializer/increment flow remains conservative, and `for-in` loop-exit joins are not narrowed. `continue`, returns, and breaks belonging to nested loops do not block this exit rule.
- A `for-in` body checks with the nullable facts active at loop entry, but body-local facts and body mutations do not leak through the loop boundary. Normal completion conservatively invalidates every fact active before the loop because the iterable may be empty or execute an arbitrary number of iterations; post-loop `for-in` exit narrowing remains unsupported. Checker failures restore the pre-loop fact state while propagating the diagnostic.
- A branch inside a loop whose last statement is `break` or `continue` is treated as non-fallthrough for the current loop body, so the condition's opposite nil-check fact is available to following statements on the path that continues. Nested loop statements remain fallthrough at the enclosing loop boundary, and this rule does not create any post-loop narrowing.
- A C-style `for` increment is flow-checked after the body, matching runtime order. The body may use facts active before the increment, the increment cannot establish a fact for the current iteration's body, and increment assignments/calls invalidate facts before the next condition or conservative loop continuation. Initializer flow remains before the condition, and post-loop joins retain their existing no-current-break rule.
- A direct field nil check on a known named-struct value, such as `box.value != nil`, narrows that field in the active branch/body. Field assignments conservatively invalidate all active nullable facts, root variable assignments invalidate related field facts, and nested fields, indexes, dynamic receivers, and alias-specific field precision remain outside this slice.
- A nil check on a statically known nested named-struct field reached through a direct variable path, such as `user.profile.score`, narrows the dotted field fact in the active branch/body. Nested field writes use the existing conservative invalidation, while dynamic receivers and alias-specific precision remain outside this slice.
- A direct array variable nil check at a compile-time integer literal or known number-binding index, such as `items[0] != nil` or `items[index] != nil`, narrows that element in the active branch/body. Index assignments and compound assignments conservatively invalidate all active nullable facts, root array assignments invalidate related index facts, and assigning the index binding invalidates facts that use it; unresolved index expressions, maps, ranges, and alias-specific index precision remain outside this slice.
- A nil check on a statically known array element reached through a direct variable, known field, or known index path, such as `matrix[0][1]`, `matrix[row][column]`, or `box.values[index]`, narrows the composed index fact in the active branch/body when every index is a literal or a known number binding. Index writes conservatively invalidate all active facts; function-call/unknown indexes, map/range elements, and alias-specific precision remain outside this slice.
...
- Named-struct `impl` blocks may declare local `operator <`, `operator <=`, `operator >`, and `operator >=` methods. Each operator has one explicit parameter of the same nominal receiver type, an explicit `bool` return annotation, and lowers through a statically resolved ordinary call. Exported structs carry canonical operator metadata through `--module-interface` and `cdi 0.1` module-cache sidecars, including generic receiver shape and resolved linkage; re-exported structs preserve it. Imported operator dispatch and independent module-product linking remain deferred, and custom structs do not satisfy generic `T: Ord` automatically.
...
- A C++ bytecode backend lowers register IR to bytecode and `.cdbc` artifacts; the default artifact is a linked program, while `--emit-module-bytecode <directory>` emits one strict `artifact: module` product per graph node with dependency insertion markers; Rust `compiler-design-vm` validates module products, links them with `link`, and executes linked programs only. Optional `--module-cache <directory> --module-rebuild-report <report.json>` reuses validated products using stable source/public-interface/dependency keys and reports private/public invalidation.
...
- The CLI requires at least one source file for normal modes and `--emit-bytecode`. Every file-backed, stdin, and virtual source is an independent module; multiple CLI files are entry modules in command-line order, and cross-file visibility requires `import` plus `export`. The legacy combined-source mode is removed. `--lsp` remains a separate stdio JSON-RPC mode. `--emit-module-bytecode <directory>` requires a module graph and writes `module-<snapshot-id>.cdbc` products without recursively lowering dependency bodies. `--module-cache` and `--module-rebuild-report` are opt-in module-product cache/measurement options and require module emission; `--module-interface-cache` may preload a cache for normal source modes as well. `--module-cache-fallback` is restricted to interface-only cache consumers and is mutually exclusive with `--module-cache-strict`. File-backed module diagnostics always report the file path with file-local line/column; only stdin keeps pathless diagnostics. Multiple entry modules aggregate their own lexer/parser errors before later stages run.
- The additive `--lsp` mode runs a stdio JSON-RPC service with full-document open/change/close synchronization, shared lexer/parser/type diagnostics, whole-document formatting edits, definition lookup, document symbols, references, hover type information for known typed expressions and callable declarations, single-document rename edits, declaration completion, and open-document workspace symbols. File-backed open URIs are analyzed through virtual frontend sources; unqualified direct imports and members selected through opened namespace aliases can navigate to declarations in other opened modules, including export-from forwarding, references can cross those same opened boundaries, value rename can return validated multi-document edits, completion can expose opened direct-import and namespace-alias exports, namespace-qualified struct/enum types plus enum variants can navigate through the same opened export chains, enum variants can complete after an opened qualified enum path, known struct receivers can complete their fields, and typed struct receivers can complete exported methods with shared signatures. Unqualified completion also includes explicit exports from other currently opened modules, with local and direct-import names taking precedence and duplicate workspace names suppressed. Closed imports and unknown/dynamic receiver completion remain later slices.
- `cargo run --manifest-path vm-rs/Cargo.toml -- trace <program.cdbc>` executes a linked artifact through the normal Rust VM and emits deterministic source-level events using existing `debug_sources`, `debug_locations`, and `debug_ranges`: function entry/exit, source-location changes, active call stacks, sorted current-frame locals, printed/returned values, and runtime failures. Trace output is on stdout as one-line records; program output is represented by `output` events and the existing runtime diagnostic remains on stderr. Interactive breakpoints and stepping are later M5D slices.
...
- Source programs construct struct values only through declared named constructors such as `Name { field: value }` and `alias.Name { field: value }`; anonymous source struct literals such as `{ field: value }` are not supported. Field access `value.name` reads an existing field; statically known non-struct field access is a type error, dynamic non-struct or missing-field access is a runtime error. Field assignment `value.name = expression` mutates an existing field, evaluates to the assigned value, and aliases observe the mutation. Statically known non-struct field assignment is a type error; dynamic non-struct targets or missing fields are runtime errors. Named struct declarations `struct Name { field: type, ... }` define static field shapes, including finite recursive nominal handles such as `optional<Node>`. Named struct type annotations check exact struct constructor initialization, and known named struct field access/assignment is statically checked. Named constructor values carry runtime type-name metadata for `typeOf`; struct assignment, parameter passing, capture, and field storage copy shared handles, while replacing a link with `nil` does not invalidate an existing handle. Struct equality and hashing remain identity-based, and repeated references on an active formatting path print `<cycle>`; strong cycles remain retained until VM teardown, with no GC or weak-reference contract. Named structs may define methods in top-level `impl Name { fun method(...) ... }` blocks. Method calls `receiver.method(args...)` are statically resolved for known named struct receiver types and lower to ordinary function calls with the receiver passed as implicit `this`; a matching user-defined method takes precedence over builtin member-call sugar for that receiver, while arrays/maps/strings/ranges retain their builtin fallback. Methods on exported/imported named structs are available through direct imports, namespace imports, and re-exports. Dynamic dispatch, inheritance, overloading, static methods, and function-valued field calls are not implemented yet.
