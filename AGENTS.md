...
- A nil check on a statically known nested named-struct field reached through a direct variable path, such as `user.profile.score`, narrows the dotted field fact in the active branch/body. Nested field writes use the existing conservative invalidation, while dynamic receivers, dynamic indexes, and alias-specific precision remain outside this slice.
- A direct array variable nil check at a compile-time integer literal index, such as `items[0] != nil`, narrows that element in the active branch/body. Index assignments and compound assignments conservatively invalidate all active nullable facts, root array assignments invalidate related index facts, and dynamic indexes, maps, ranges, and alias-specific index precision remain outside this slice.
- A nil check on a statically known array element reached through a direct variable, known field, or known compile-time index path, such as `matrix[0][1]` or `box.values[0]`, narrows the composed index fact in the active branch/body. Index writes conservatively invalidate all active facts; dynamic indexes, map/range elements, and alias-specific precision remain outside this slice.
