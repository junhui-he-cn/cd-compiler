...
- Named-struct `impl` blocks may declare local `operator <`, `operator <=`, `operator >`, and `operator >=` methods. Each operator has one explicit parameter of the same nominal receiver type, an explicit `bool` return annotation, and lowers through a statically resolved ordinary call. This slice is limited to the struct's defining compilation unit and linked `.cdbc` path; operator metadata is not yet exported through `.cdi` or module cache products, and custom structs do not satisfy generic `T: Ord` automatically.
