...
- Function and closure bodies are checked without inheriting nullable narrowing from their definition-site branch; captured nullable bindings require a fresh supported nil check inside the body. Direct captured-call and indirect/dynamic-call effects are handled after successful call checking; native callbacks and struct-method effects remain conservative.
- A resolved direct call to a local function with captured bindings invalidates active nullable narrowing for those visible captured bindings after argument checking. Non-capturing calls keep unrelated facts; native callbacks and struct-method effects remain conservative and are deferred to later M2A slices.
- An indirect or dynamic function call, including a function alias or function-valued binding that is not resolved to a direct `FunctionStmt`, invalidates all active nullable narrowing after successful argument checking. Native callbacks and struct-method effects remain conservative and are deferred to later M2A slices.
