...
- The additive `--lsp` mode runs a stdio JSON-RPC service with full-document open/change/close synchronization, shared lexer/parser/type diagnostics, whole-document formatting edits, definition lookup, document symbols, and references. The first boundary is intentionally single-document in semantic scope and uses the existing stdin import restriction; completion, hover/type information, and cross-module navigation are not implemented yet.
