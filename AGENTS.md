...
- The additive `--lsp` mode runs a stdio JSON-RPC service with full-document open/change/close synchronization, shared lexer/parser/type diagnostics, whole-document formatting edits, definition lookup, and document symbols. The first boundary is intentionally single-document in semantic scope and uses the existing stdin import restriction; references, completion, and cross-module navigation are not implemented yet.
