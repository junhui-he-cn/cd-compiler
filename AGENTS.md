...
- A C-style `for` condition's true-branch nullable narrowing is active while checking its body. The existing initializer/increment checks and loop boundary remain conservative; `for-in` and loop-exit joins are not narrowed.
