...
| LRU 缓存 `LruCache<K,V>` | 现有（S6，数组版） | 容量限制、`get`、`put`、淘汰最久未用项 | 键/值数组；`O(n)` 查找与移动，零容量拒绝写入 |
| LFU 缓存 `LfuCache<K,V>` | 现有（S6，数组版） | 按访问频率淘汰、同频按最近使用淘汰 | 频次 + 最近使用时钟；线性查找与淘汰，操作 `O(n)` |
...
数值路径和，以及基于它的基础 BST 操作；`AvlTree<T>` 使用数组节点实现了
可变旋转和平衡维护。序列化和 LCA 仍属于后续专题。
...
已实现 Trie、Fenwick 树、不可变二叉树、BST、基础线段树和数组节点版
`AvlTree<T>`；红黑树及其他可变平衡树再根据实际需求决定。
...
移动为 `O(n)`。`LruCache<K: Eq, V>` 也已提供数组版本，按最久未使用到最近
使用保存条目，访问和更新刷新顺序，满容量插入淘汰首项；由于线性查找和数组移动，
核心操作为 `O(n)`。`OrderedMap<K, V>` 使用同样的有序数组策略，按 comparator
等价键保留一个条目，`put` 更新值但保留首次键和排序位置。`BiMap<K: Eq, V: Eq>`
采用双方唯一且冲突拒绝的数组实现，按 key/value 双向查询，删除时同步移除配对条目。
`LfuCache<K: Eq, V>` 也已提供数组版本，按最低频次淘汰，同频时淘汰最久未使用项；
`get` 和更新会增加频次并刷新使用时钟。`AvlTree<T>` 使用并行数组保存节点和
私有整数索引，旋转、插入和删除保持 `O(log n)`，不向调用者暴露稳定节点引用。
其余红黑树、LRU 高性能节点版、LFU 高性能桶/索引版、跳表、哈希/位集、后缀结构和几何算法，
...
4. **递归结构体。** 近期仍不支持递归结构体、节点指针或等价的可变递归引用。
   不可变链表和树使用递归枚举；可变图或树使用节点数组加整数索引。`AvlTree<T>`
   已采用不暴露节点索引的并行数组表示；可变链表、双向链表、环检测、红黑树和
   高效 LRU 仍等待稳定节点引用与别名契约。
