...
边界。Phase 12 的第三片已完成：选择性导入可以为结构体和枚举指定本地别名，别名贯通
类型注解、构造器、方法、字段和模式匹配，同时保留导出方的名义类型身份。
类型位置的 `T?` 已作为独立兼容语法糖加入，`optional<T>` 仍是规范化的 AST 和接口形状。
逗号分隔列表也已统一支持单个尾逗号，且格式化器保留源文件中的尾逗号。
数字字面量已支持数字之间的单个下划线和 `e`/`E` 指数形式，仍统一归一化为 `number`。
注释已支持 `//` 与非嵌套 `/* ... */`；`///` 仍按普通行注释处理，不生成文档 AST。
