...
**状态（第十六 inline native arguments 窄切片已完成，2026-07-31）：**
`NativeCall` 的零、一、二参数直接使用内部 `NativeArguments` inline variant，三
参数以上保留 `Many(Vec<Value>)` fallback；`execute_native_call(name, Vec<Value>)`
入口和 native arity、callback、资源、profile、诊断语义不变。native 小 workload
仍处于启动噪声范围；详见
...
**状态（第二十 cached instruction-checkpoint mode 窄切片已完成，2026-07-31）：**
`RunConfig` 在 VM 创建时预计算 bounded/unlimited 与 cancellation 的四种内部
checkpoint mode；每条指令不再重复读取两个 public `Option`，取消检查、limit
顺序、溢出和错误语义保持不变。11 次同序列对照的 `execution_loop` median 从
`0.425305s` 降至 `0.413971s`，`execution_closure` 从 `0.254038s` 降至
`0.253021s`；详见
...
**状态（第二十三 borrowed-Print-operand 窄切片已完成，2026-07-31）：** `Print`
指令通过 `read_register_ref` 借用只读寄存器值，避免格式化前的 `Value` clone；
UTF-8 输出预算、partial-output suppression、trace/debug、profile、错误和
`.cdbc 0.1` 行为保持不变。focused benchmark 只作为 no-regression 证据，不建立
timing threshold；详见
[`docs/decisions/vm-execution-loop-023-borrow-print-operand.md`](decisions/vm-execution-loop-023-borrow-print-operand.md)。

**状态（第二十四 borrowed-Field-receiver 窄切片已完成，2026-07-31）：** `Field`
指令借用只读 struct receiver，避免 named-field lookup 前的 `Value` clone；选中
field value 仍以 owned 结果写入 destination，field assignment、alias、错误和
`.cdbc 0.1` 行为不变。当前没有 scaled field benchmark，因此不建立 timing
threshold；详见
...
**状态（第二十六 borrowed-primitive-comparison-operands 窄切片已完成，2026-08-01）：**
ordered comparison 的 number/string operands 通过 `read_register_ref` 借用，named-
struct witness 在可重入调用前仍显式 clone 两个 struct 参数；数值、Unicode ordering、
witness dispatch、错误、资源、profile/trace/debug 和 `.cdbc 0.1` 行为不变。复用
`execution_loop` comparison workload 作 no-regression 证据，不建立跨平台 timing
threshold；详见
[`docs/decisions/vm-execution-loop-026-borrow-comparison-operands.md`](decisions/vm-execution-loop-026-borrow-comparison-operands.md)。

**状态（第二十七 borrowed-AssertArray-operand 窄切片已完成，2026-08-01）：**
`AssertArray` 借用 source register；array/range 仍只为 destination 保留浅 clone，
map 先在 borrowed storage 上快照 insertion-ordered keys，再创建 fresh array，避免
复制 map `Value`。for-in 顺序、alias、资源、profile/trace/debug 和 `.cdbc 0.1` 行为
不变；`Call`、native callback 与 mutable paths 继续保持 owned boundary；详见
[`docs/decisions/vm-execution-loop-027-borrow-assert-array-operand.md`](decisions/vm-execution-loop-027-borrow-assert-array-operand.md)。
...
**状态（第四 large-map payload 窄切片已完成，2026-08-01）：** capacity corpus
新增 2,048-entry map 的 C++ emission、Rust length/key lookup 和
`--max-elements 1024` rejection；artifact size、map order、string-key lookup、
...
rejection；named-struct identity、alias、stdout、resource error 和 `.cdbc 0.1` 保持不变，elapsed/peak RSS
仍只作同机观察；详见
[`docs/decisions/vm-capacity-005-nested-struct.md`](decisions/vm-capacity-005-nested-struct.md)。

**状态（第六 aggregate-allocation-churn 窄切片已完成，2026-08-02）：** capacity
corpus 新增 512 次 array/map/named-struct replacement，固定默认和
`--max-elements 4104` exact success、`4103` cumulative runtime-element rejection，
以及 failure 前 stdout 为空；allocation identity、alias、resource error、host RSS
...
**状态（第二 centralized arity validation 窄切片已完成，2026-07-31）：**
`NativeSpec` 现在携带每个 native 的精确 legacy arity 诊断，公共 dispatch
边界在 profile hook 之后统一执行 `min_arity/max_arity` 校验，helper 不再重复
数量判断；callback 签名、range 语义、资源 checkpoint 和所有既有错误文本保持
不变。详见
[`docs/decisions/vm-native-registry-002-arity.md`](decisions/vm-native-registry-002-arity.md)。

下一步只评估有 workload 证据的 resource-cost metadata；在 checkpoint 时序和
分配/回调预算无法由统一策略覆盖前，资源策略继续留在各 helper 中。

**状态（第三 resource touchpoint profile 窄切片已完成，2026-07-31）：**
`NativeSpec` 现在以 `None`、`InstructionCheckpoints`、`RuntimeElements` 或
`Both` 描述现有预算 hook 的可能触点；该字段只读、不计费，`contains` 等按
值形状变化的 native 使用保守分类，实际 checkpoint、取消优先级和元素预算仍
由 helper 执行。详见
[`docs/decisions/vm-native-registry-003-resource-profile.md`](decisions/vm-native-registry-003-resource-profile.md)。

下一材料边界是 parameter/return constraints：当前 `.cdbc 0.1` native call
只携带 name-table reference，若要让 VM 读取类型描述必须先决定共享 artifact
契约或 versioned native ID；在该决定前不引入隐式 host ABI。

**状态（第四 signature shape metadata 窄切片已完成，2026-07-31）：**
`NativeSpec` 现在为全部 29 个 native 记录私有 `NativeSignature`，覆盖数组、
map、数字、字符串、map key、callback 参数/返回形状及 native result 形状；
`range` 的重复数字参数和 `contains` 的值形状差异保持保守描述。该元数据不
序列化、不集中校验，helper 仍是动态运行时检查的唯一执行权威。详见
[`docs/decisions/vm-native-registry-004-signature-shapes.md`](decisions/vm-native-registry-004-signature-shapes.md)。

下一边界仍是将形状变成 centralized runtime validation 或 artifact constraint；
这需要 successor artifact field/version 或 versioned native-ID contract，当前
不改变 `.cdbc 0.1`。
