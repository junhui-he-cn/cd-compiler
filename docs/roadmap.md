...
`M2A-FLOW-017`. Direct-variable assignment invalidation now makes nullable
...
arm states. Broader loop-exit and nested/dynamic index analysis remain
subsequent slices. Direct named-struct field and compile-time integer array-index
checks now use the same target flow facts with conservative mutation
...
loop body, while broader `for-in` loop-exit joins remain conservative.
...
implemented. The active near-term slice is M2A-FLOW-017: loop-control branch
facts for `break` and `continue`. Subsequent M2A slices must retain the same
