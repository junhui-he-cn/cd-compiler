...
`M2A-FLOW-009`. Direct-variable assignment invalidation now makes nullable
...
- decide recursive type support, richer narrowing, and remaining known
  ...
  Richer loop-exit, C-style `for`, `for-in`, both-fall-through explicit-else,
...
| Broader loop-exit, C-style `for`, `for-in`, both-fall-through explicit-else, field, and index nullable narrowing | Intentionally reopened for an M2 decision; returning-arm guards and `while` body facts are the currently admitted loop/branch slices. |
...
implemented. The active near-term slice is M2A-FLOW-009: `while` body
...
implemented; M2A-FLOW-001 through M2A-FLOW-009 are the active semantic slices.
