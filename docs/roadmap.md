...
| Language | Functions, closures, generics and constraints, `optional<T>`/`T?`, trailing commas, numeric separators/exponents, line/block comments, half-open `..<` ranges, legacy C-style `for`, string/array/map/range `for-in`, enums/patterns, named and recursive structs, collection semantics, `Eq`/`Ord`/`Hash` capabilities |
...
Phase 20 is now in progress. Its first narrow slice extends the existing
VM-internal iterator protocol to strings: `for character in text` snapshots
Unicode scalar values and binds each element as `string`. Generic
`Iterable<T>`/`Iterator<T>` declarations and user-defined iterator behavior
remain deferred until a separate protocol decision.

See [`language-iterator-string-001`](decisions/language-iterator-string-001.md).
