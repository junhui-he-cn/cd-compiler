...
`M2A-FLOW-018`. Direct-variable assignment invalidation now makes nullable
...
runtime order, while broader `for-in` loop-exit joins remain conservative.
...
implemented. The active near-term slice is M2A-FLOW-018: runtime-ordered
C-style `for` increment flow. Subsequent M2A slices must retain the same
