...
body has no current-loop `break`; C-style `for`, `for-in`, and broader loop-exit
...
implemented. The active near-term slice is M2A-FLOW-014: no-break `while`
condition-failure exit narrowing. Subsequent M2A slices must retain the same
...
implemented; M2A-FLOW-001 through M2A-FLOW-014 are the active semantic slices.
