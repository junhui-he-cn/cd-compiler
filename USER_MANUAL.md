...
格式化器从生产 lexer/parser 的无损 token/trivia 视图生成稳定源码：默认使用两格缩进，保留字符串和注释文本，最多保留一个空白行，并对过宽的逗号列表使用固定规则换行。它不会把字符串或注释拆开，也不会自动插入或删除尾逗号。
...
程序由声明和语句组成。语句使用分号结束，代码块使用大括号；注释使用 `//` 或非嵌套的
`/* ... */`，字符串使用双引号。`///` 暂时按普通行注释处理，不生成文档 AST。当前没有自动
分号插入。

字符串支持 `\n`、`\r`、`\t`、`\0`、`\\` 和 `\"` 转义；不允许裸换行或未转义的双引号。`.cdbc` 字节码产物文本层使用的转义是独立格式，不属于源语言语法。

词法规则的实用摘要：标识符由 ASCII 字母、数字和下划线组成，但不能以数字开头；数字字面量
支持数字之间带单个下划线的整数/小数和 `e`/`E` 指数形式；负数是对数字使用一元 `-` 运算符。
字符串在一对双引号之间，支持 `\n`、`\r`、`\t`、`\0`、`\\` 和 `\"` 转义；字符串内不允许裸
换行，未知转义会在词法阶段报错。未闭合块注释、错误数字分隔符和不完整指数同样是词法错误。
关键字包括 `let`、`fun`、`struct`、`enum`、`impl`、`match`、`if`、`else`、`while`、`for`、
`break`、`continue`、`return`、`print`、`import`、`export`、`private`、`true`、`false` 和 `nil`。
...
`number` 仍是唯一的数值类型，并以 `f64` 保存。整数部分、小数部分和指数部分都可以
在数字之间使用单个下划线分隔，例如 `1_000_000`、`3.141_592`、`1e10`、`1e-6` 和
`2.5E10`。下划线不能出现在数字开头、结尾或连续出现；指数必须至少包含一个数字，
这些错误会在词法阶段报告。
...
`&&` 和 `||` 会短路，两个操作数都必须是 `bool`，结果类型是 `bool`，不再返回被选中的操作数。
...
`export answer;` 或 `export User;`。导出多个已定义名称仍可以写成
`export answer, User;`。转导出可以写成 `export answer from "./lib.cd";`，但转导出的名字不会自动成为当前模块的本地名字；如果转导出模块也需要使用该名称，应另外写 `import`。相同规范路径的重复导入会被去重；不能从 stdin 导入。
