...
Static generic capability bounds `T: Eq` and `T: Ord` are also supported. `Eq`
  permits `==` and `!=` in generic code; `Ord` permits the numeric ordering
  operators and currently admits `number`, while also satisfying `Eq`. These
  bounds are compile-time checks only: they do not introduce trait objects,
  dynamic dispatch, or user-defined capability implementations.
