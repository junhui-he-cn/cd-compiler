...
int main()
{
    ...
    assert(SemanticTypes::compatible(t, typeParameterType("T")));
    assert(!SemanticTypes::compatible(t, typeParameterType("U")));

    const TypeInfo number = simpleType(StaticType::Number);
    const TypeInfo boundedT = typeParameterType("T", number);
    assert(SemanticTypes::compatible(number, boundedT));
    assert(!SemanticTypes::compatible(simpleType(StaticType::String), boundedT));
    ...
    assert(SemanticTypes::compatible(boxedNumber, namedStructType("Box", {number})));
    assert(!SemanticTypes::compatible(boxedNumber, namedStructType("Box", {simpleType(StaticType::String)})));

    const TypeInfo nested = arrayType(t);
    assert(SemanticTypes::compatible(nested, arrayType(typeParameterType("T"))));
    assert(!SemanticTypes::compatible(nested, arrayType(simpleType(StaticType::Number))));
    ...
    assert(SemanticTypes::compatible(
        stringNumberMap,
    ...
    assert(SemanticTypes::compatible(range, simpleType(StaticType::Range)));
    assert(!SemanticTypes::compatible(range, simpleType(StaticType::Array)));
    ...
    assert(typeInfoName(SemanticTypes::substituteTypeParameters(t, substitutions)) == "number");
